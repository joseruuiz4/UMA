...
public class PersonaController {
    ...
    @PostMapping(value ="/persona/{id}/logo",     consumes = { MediaType.MULTIPART_FORM_DATA_VALUE, MediaType.APPLICATION_JSON_VALUE} )
	public ResponseEntity<?> uploadImage(@RequestPart("logo") MultipartFile logo) {
		return ResponseEntity.ok("Received "+logo.getOriginalFilename());
	}
	...
	public ResponseEntity<?> savePersona(@RequestBody Persona persona) {
        try{
            ...
            return ResponseEntity.status(201).body("Una nueva Persona se ha anyadido");
        }
        catch(Exception e){
