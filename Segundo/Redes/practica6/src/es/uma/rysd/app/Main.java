...
public class Main {	
	...
	public static void largest(SWClient sw) {
		// Getting the number of stored vehicles
		int max_v = sw.countResources("vehicles");
		if (max_v == 0) {
		...
		System.out.println("Generating new question...");
		// Picking two random vehicles without repeating
		List<Integer> used = new ArrayList<Integer>();
		List<Vehicle> vehicles = new ArrayList<Vehicle>();
		int counter = 0;
		while (counter < 2) {
			Integer p = getRandomResource(max_v, used);
			Vehicle vehicle = sw.getVehicle(sw.buildResourceUrl("vehicles", p));
			if (vehicle == null) {
				System.out.println("Hubo un error al encontrar el recurso " + p);
			} else {
		...
		Integer n = null;
		do {
			System.out.println("Cual es mas largo[0] "+ vehicles.get(0).name + " o [1] " + vehicles.get(1).name);
			try {
				n = Integer.parseInt(sc.nextLine());
			} catch (NumberFormatException ex) {
				n = -1;
		...
		for (Vehicle p : vehicles) {
			System.out.println(p.name + " mide " + p.length);
		}

		// Result
		if(Double.parseDouble(vehicles.get(n).length) >= Double.parseDouble(vehicles.get((n+1)%2).length)){
			System.out.println("Enhorabuena!!! "+ success[rand.nextInt(success.length)]);
		} else {
			System.out.println("Fallaste :( " + error[rand.nextInt(error.length)]);
		}
